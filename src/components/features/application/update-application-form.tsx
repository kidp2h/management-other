...
import { UploadedFilesCard } from '@/components/common/uploaded-files-card';
...
export interface UpdateApplicationFormProps {
  isDialog?: boolean;
  defaultValues?: UpdateApplicationSchema;
  onSuccess: () => void;
  data: any;
}
        ...
        startUpdateTransition(async () => {
          ...
          toast.promise(
            onUpload(values.files, async uploaded => {
              ...
              const mappedUploaded = uploaded.map(
                file => `${file.appUrl}|${file.name}`,
              );

              const remainingFiles = originalFiles.filter(
                (file: string) => !filesRemove.includes(file),
              );
              const newFiles = remainingFiles.concat(mappedUploaded);
              // const newFiles = originalFiles.filter(file => file != file)
              ...
              onSuccess();
              toast.success('Đơn đã được cập nhật');
              setLoading(false);
            }),
            {
              loading: 'Đang tải tệp lên',
              success: () => {
                // form.reset();
    ...
    <div className="flex flex-col gap-5">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Button
            type="submit"
            disabled={isUpdatePending || loading}
